...
shinyServer(function(input, output) {
    ...
    output$res_plot1 <- renderPlot({
        ...
        seletedkey <- dparams[dparams$normq == input$bw_q & dparams$normLen == input$bw_Len & dparams$normVmax2 == input$bw_Vmax2 & dparams$normRUE == bw_RUE & dparams$normMaxFix == bw_Fix, "keysc"]
    ...
    output$table1 <- renderDataTable({
        ...
        if (input$groupN1 == "Legume") 
        {bw_Fix <- 1
        val_init <- c(3.4875, 0.05, 3.22, 24.)
        } else 
        {bw_Fix <- 0
        val_init <- c(3.4875, 0.05, 3.22, 0.)
        }
        
        var_ <- c("Len", "q", "Vmax2",  "MaxFix")
        dat_ <- dparams[dparams$normq == input$bw_q & dparams$normLen == input$bw_Len & dparams$normVmax2 == input$bw_Vmax2 & dparams$normRUE == bw_RUE & dparams$normMaxFix == bw_Fix, var_]
        
        delta <- as.numeric(dat_)
        df <- data.frame(Parameters = var_, Sp1=val_init, Sp2=val_init + delta, delta_Sp2Sp1=delta)
        #corriger pour MaxFix... delta autre sens
